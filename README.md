...
├── .runtime-data/               # Локальные bind-mounted данные runtime (создаётся Docker, в git не хранится)
...
## Локальное хранение БД

Для `microservice_account` и `microservice_data` PostgreSQL по умолчанию
хранит данные в каталоге репозитория, а не в Docker named volumes:
...
- данные живут на том же диске, где лежит репозиторий;
