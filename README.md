...
# после backend restart проверь private/WG path
...
- если на admin-host менялся только backend host/IP или нужно просто поднять уже собранный `admin-online`, используй `start ... onlyadmin <wg-ip>` без rebuild
- для split deployment используй WG/private IP или private DNS backend-хоста, а не public IP
