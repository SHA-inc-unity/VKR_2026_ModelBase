...
./start.sh all onlyadmin 10.44.0.1 # сразу записать backend host/IP для admin-online
...
./restart.sh all onlyadmin 10.44.0.1 # обновить backend host/IP и пересобрать admin-online
...
Канонический browser URL для этого режима: `http://<admin-host>:8501/admin/`.
Не используй как ориентир bare `http://<admin-host>:8501/`: `admin-online`
работает с `basePath=/admin`. На backend-хосте в режиме `noadmin` порт
`8501` не является UI-входом admin-панели.

Важное уточнение: `admin-online` в текущем compose не делает TLS-терминацию.
Он публикует plain HTTP `8501:3000`, поэтому рабочий URL по умолчанию —
именно `http://<admin-host>:8501/admin/`. Для `https://...:8501/admin/`
нужен отдельный reverse proxy или внешний TLS-terminator перед этим портом.

Теперь launcher умеет настраивать этот режим от одного backend host/IP:

- Linux: `./start.sh all onlyadmin 10.44.0.1`
- PowerShell: `.\start.ps1 -Mode onlyadmin -BackendHost 10.44.0.1`

Если backend host/IP не передан аргументом, launcher теперь спрашивает его
сразу в консоли до `docker compose`, как и другие обязательные интерактивные
значения первого запуска, и сохраняет в `microservice_admin/.env` как
`ONLINE_BACKEND_HOST`, а затем автоматически выводит из него:
...
Это убирает ручное редактирование всех `ONLINE_*` при запуске remote admin-head.
...
admin-host: `http://<admin-host>:8501/admin/`; backend-host:8501 остаётся
