...
- bash-версии `start.sh` / `restart.sh` перед `docker compose up` теперь делают preflight host-port check для publish-единиц launcher-а (`8443`, `8501`, `7510`, `7520`, infra ports и `ADMIN_HTTP_PORT` / `ADMIN_HTTPS_PORT` в `onlyadmin`) и падают сразу с явным сообщением, если порт занят внешним контейнером/процессом
...
В режиме `onlyadmin` используются compose-сервисы `admin-online` и
`admin-online-proxy`. `admin-online` остаётся внутренним Next.js runtime на
`3000`, а `admin-online-proxy` публикует `80/443` (`ADMIN_HTTP_PORT` /
`ADMIN_HTTPS_PORT`) и читает внешние адреса из namespace
`ONLINE_*` (`ONLINE_KAFKA_BOOTSTRAP_SERVERS`, `ONLINE_REDPANDA_ADMIN_URL`,
`ONLINE_ACCOUNT_URL`, `ONLINE_GATEWAY_URL`, `ONLINE_MINIO_URL`,
`ONLINE_REDIS_URL`). Для HTTPS facade launcher дополнительно хранит в
`microservice_admin/.env` `ADMIN_BACKEND_BASE_URL`; сам доступ к facade выполняется через admin JWT после входа в admin-панель.

Primary split path идёт через backend HTTPS facade на `ADMIN_BACKEND_BASE_URL`.

Канонические browser URL для этого режима: `https://sha-trade.tech/admin/`
и `https://www.sha-trade.tech/admin/`.
Не используй как ориентир bare `https://sha-trade.tech/`: `admin-online`
...
- зеркалит derived backend endpoints ещё и в direct runtime env `KAFKA_BOOTSTRAP_SERVERS`, `REDPANDA_ADMIN_URL`, `ACCOUNT_URL`, `GATEWAY_URL`, `MINIO_URL`, `BACKEND_CONNECTION_TARGET`, чтобы `admin-online` и локальный `npm run dev/start` не требовали ручной правки `microservice_admin/.env`
- если `ADMIN_BACKEND_TLS_INSECURE` ещё пустой и facade URL начинается с `https://`, записывает `ADMIN_BACKEND_TLS_INSECURE=1`, чтобы autogenerated self-signed cert backend-host принимался Node runtime admin-head
- если public onlyadmin keys ещё пустые, дописывает `ADMIN_HTTP_PORT=80`, `ADMIN_HTTPS_PORT=443`, `ADMIN_PRIMARY_DOMAIN=sha-trade.tech`, `ADMIN_SECONDARY_DOMAIN=www.sha-trade.tech`, `ADMIN_TLS_CERT_PATH=/etc/letsencrypt/live/sha-trade.tech/fullchain.pem` и `ADMIN_TLS_KEY_PATH=/etc/letsencrypt/live/sha-trade.tech/privkey.pem`
- прокидывает эти ключи в `admin-online`, чтобы split mode действительно переключился с direct Kafka path на backend facade
- печатает в консоль итоговую admin-конфигурацию (`ONLINE_*`, прямые runtime URL и facade/public defaults), чтобы `start.sh` / `restart.sh` не требовали ручного просмотра `.env`
...
- `start ... onlyadmin` делает только `docker compose --profile online up -d admin-online admin-online-proxy` без принудительной пересборки образа
- `restart ... onlyadmin` по-прежнему делает rebuild и затем поднимает `admin-online admin-online-proxy`
...
режим `onlyadmin` поднимает `admin-online` + `admin-online-proxy`, который
слушает `80/443`, редиректит bare `/` на `/admin/` и использует cert/key с
admin-host. То есть в split deployment UI надо открывать именно на
admin-host: `https://sha-trade.tech/admin/` или `https://www.sha-trade.tech/admin/`; backend-host:8501 остаётся
