...
# → поднимает WireGuard-сервер + wstunnel-server на TCP 8443 или первый свободный fallback-порт и печатает JOIN TOKEN
...
Launcher автоматически декодирует join token, переносит WebSocket metadata в `microservice_admin/.env`, поднимает `wstunnel-client` + `vpn-client`, ждёт туннеля и выставляет `ONLINE_*` на `10.44.0.1:*`. Подробнее: [../microservice_infra/VPN_CONTAINERIZED.md](../microservice_infra/VPN_CONTAINERIZED.md).

Если backend-host уже слушает `VPN_WS_PORT` (`8443` по умолчанию), `noadmin + VPN`
shell launcher заранее проверяет порт через `ss`/`netstat`, выбирает первый свободный
fallback из `8443 18443 28443 38443 48443 58443`, записывает его в
`microservice_infra/.env` и печатает этот порт в JOIN TOKEN-блоке. Список можно
переопределить переменной `MODELLINE_VPN_WS_PORT_CANDIDATES` перед запуском.

При `onlyadmin <JOIN_TOKEN>` и при повторном `onlyadmin` с уже сохранённым
`wg0.conf` launcher теперь делает `--force-recreate` для `wstunnel-client` и `vpn-client`.
...
Повторный `onlyadmin` теперь также совместим с legacy `wg0.conf`, который был
сохранён ещё до появления `# VPN_*` metadata-комментариев в join token.
Отсутствие этих строк больше не роняет shell launcher под `set -e`: он
использует старый `Endpoint` как fallback, переносит transport env в `.env`
и переписывает peer `Endpoint` на локальный `127.0.0.1:<VPN_CLIENT_LOCAL_PORT>`.

В `noadmin + VPN` shell launcher теперь также заранее подбирает свободный
`VPN_WS_PORT` для `wstunnel-server` и прописывает backend-side env для private
path: `REDPANDA_EXTERNAL_HOST=10.44.0.1`,
