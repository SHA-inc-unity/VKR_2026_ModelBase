...
| `cmd.data.pairs.list` | in | req/reply | **Currency pairs center.** `{ bases:[{asset,active}], quotes:[{asset,active}], symbols:[…] }`. `symbols` = active base × active quote cross-product (source of truth for MarketWatcher + dataset config + gateway/app) |
| `cmd.data.pairs.add` / `.remove` / `.set_active` | in | req/reply | Mutate the base/quote vocabularies (`{role:'base'\|'quote', asset[, active]}`); reply with the full state, publish `events.data.pairs.changed`, and hot-reload the market watcher |
| `cmd.data.updates.list` | in | req/reply | **App updates / changelog** (append-only store). Empty payload `{}` → `{ releases: [...] }` (or `{ error }`), newest release first. Each release: `{ version, date, title, highlights[], builds:[{ build, date, changes:[{ type, scope, text }] }] }` — builds ordered `build_number DESC`, changes in authored order; `type ∈ {feature,fix,improvement}`, `scope ∈ {frontend,backend,internal}`. Served from the immutable `app_update_release` / `app_update_change` tables (DB-level trigger guard blocks UPDATE/DELETE/TRUNCATE even for the owner; seeded once from the Flutter client's bundled changelog). Consumed by gateway `GET /api/updates`. |
| `events.data.ingest.progress` | out | event | Staged ingest progress (fire-and-forget, no reply) |
| `events.data.pairs.changed` | out | event | Emitted on any pair mutation; the market watcher re-discovers + re-subscribes without a restart |
...
4. Fetches market feeds in parallel through the selected exchange client.
    ...
    - **Kraken** uses `/0/public/OHLC`; pair resolution goes through
      ...
      inside the client. For reachable windows Kraken first tries one OHLC
      request for the whole clipped range and only falls back to smaller paged
      windows when the single response is incomplete. The fetch window is
      clipped to Kraken's reachable lookback before `FindMissingTimestampsAsync`,
      so impossible historical ranges complete as a no-op instead of retrying
      forever.
...
hydrates the authoritative closed raw candle through the exchange client and
upserts `OHLCV + funding_rate + open_interest + RSI` into the canonical dataset
table for that `exchange + symbol + timeframe`, while still persisting one row per
...
the ingest / repair / upsert paths.
...
- `BybitSymbolsPerSubscription`
- `KrakenSymbolsPerSubscription`
...
| `candle_body` | `abs(close - open) / close` | open |
