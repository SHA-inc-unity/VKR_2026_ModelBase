        ...
        EXECUTE format($sql$
            WITH raw_win AS (
                SELECT
                    timestamp_utc,
                    -- returns
                    (close_price::double precision / NULLIF(LAG(close_price::double precision, 1)  OVER w, 0) - 1)::double precision AS return_1,
                    (close_price::double precision / NULLIF(LAG(close_price::double precision, 6)  OVER w, 0) - 1)::double precision AS return_6,
                    (close_price::double precision / NULLIF(LAG(close_price::double precision, 24) OVER w, 0) - 1)::double precision AS return_24,
                    -- log returns
                    LN(GREATEST(close_price::double precision / NULLIF(LAG(close_price::double precision, 1)  OVER w, 0), 1e-10))::double precision AS log_return_1,
                    LN(GREATEST(close_price::double precision / NULLIF(LAG(close_price::double precision, 6)  OVER w, 0), 1e-10))::double precision AS log_return_6,
                    LN(GREATEST(close_price::double precision / NULLIF(LAG(close_price::double precision, 24) OVER w, 0), 1e-10))::double precision AS log_return_24,
                    -- rolling 6
                    AVG(close_price::double precision)       OVER (PARTITION BY symbol, timeframe ORDER BY timestamp_utc ROWS BETWEEN 5  PRECEDING AND CURRENT ROW) AS roll6_mean,
                    STDDEV_POP(close_price::double precision) OVER (PARTITION BY symbol, timeframe ORDER BY timestamp_utc ROWS BETWEEN 5  PRECEDING AND CURRENT ROW) AS roll6_std,
                    MIN(close_price::double precision)       OVER (PARTITION BY symbol, timeframe ORDER BY timestamp_utc ROWS BETWEEN 5  PRECEDING AND CURRENT ROW) AS roll6_min,
                    MAX(close_price::double precision)       OVER (PARTITION BY symbol, timeframe ORDER BY timestamp_utc ROWS BETWEEN 5  PRECEDING AND CURRENT ROW) AS roll6_max,
                    -- rolling 24
                    AVG(close_price::double precision)       OVER (PARTITION BY symbol, timeframe ORDER BY timestamp_utc ROWS BETWEEN 23 PRECEDING AND CURRENT ROW) AS roll24_mean,
                    STDDEV_POP(close_price::double precision) OVER (PARTITION BY symbol, timeframe ORDER BY timestamp_utc ROWS BETWEEN 23 PRECEDING AND CURRENT ROW) AS roll24_std,
                    MIN(close_price::double precision)       OVER (PARTITION BY symbol, timeframe ORDER BY timestamp_utc ROWS BETWEEN 23 PRECEDING AND CURRENT ROW) AS roll24_min,
                    MAX(close_price::double precision)       OVER (PARTITION BY symbol, timeframe ORDER BY timestamp_utc ROWS BETWEEN 23 PRECEDING AND CURRENT ROW) AS roll24_max,
                    -- OI
            ...
            UPDATE %I AS t SET
                ...
                price_to_roll6_mean  = (close_price::double precision / NULLIF(r.roll6_mean,  0))::double precision,
                price_to_roll24_mean = (close_price::double precision / NULLIF(r.roll24_mean, 0))::double precision,
