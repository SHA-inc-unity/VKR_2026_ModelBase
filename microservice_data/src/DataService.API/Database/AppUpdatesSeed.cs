...
public static class AppUpdatesSeed
{
    ...
    public static readonly IReadOnlyList<SeedRelease> Releases = new List<SeedRelease>
    {
        new(
            ...
            Builds: new[]
            {
                new SeedBuild(59, "2026-06-03", Changes(
                    (TypeImprovement, ScopeFrontend,
                        "Переделан весь UI уведомлений: яркая активная кнопка «Создать уведомление»; карточки алертов с иконкой монеты и направлением (выше/ниже); современная лента уведомлений с цветными иконками по типу и отметкой непрочитанного; сгруппированные настройки (push / что присылать / порог цены)"))),
                new SeedBuild(58, "2026-06-03", Changes(
                    (TypeImprovement, ScopeFrontend,
                        "Экран алертов: форма создания больше не открывается сама при заходе с монеты — теперь по кнопке «+» (символ монеты подставляется автоматически); пункт переименован в «Создать уведомление»"))),
