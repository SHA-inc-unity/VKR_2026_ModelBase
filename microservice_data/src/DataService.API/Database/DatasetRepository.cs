...
///   exchange        VARCHAR
///   timeframe       VARCHAR
///   index_price     NUMERIC   (close price)
///   open_price      NUMERIC
///   high_price      NUMERIC
///   low_price       NUMERIC
///   volume          NUMERIC
...
public sealed class DatasetRepository
{
        ...
        for (int offset = 0; offset < rows.Count; offset += batchSize)
        {
            ...
            var sql = $@"
                INSERT INTO ""{tbl}"" (
                    ...
                    index_price, open_price, high_price, low_price, volume, turnover,
    ...
    /// <summary>
    /// Returns a page of raw rows from the table together with the total row count.
    ...
    /// </summary>
    public async Task<(long TotalRows, IReadOnlyList<IDictionary<string, object?>> Rows)>
        BrowseRowsAsync(
            string tableName,
            int    page,
            int    pageSize,
            ...
            CancellationToken ct = default)
    {
        if (!await TableExistsAsync(tableName, ct))
            return (0, Array.Empty<IDictionary<string, object?>>());
        ...
        long total = await conn.ExecuteScalarAsync<long>(
            new CommandDefinition(
        ...
        string tableName, CancellationToken ct = default)
    {
        ...
        await conn.ExecuteAsync(new CommandDefinition(alterSb.ToString(), cancellationToken: ct));
        ...
        foreach (var h in DatasetConstants.ReturnHorizons)
            parts.Add($"{PctChange("index_price", h)} AS return_{h}");
        foreach (var h in DatasetConstants.ReturnHorizons)
            parts.Add($"{LogReturn("index_price", h)} AS log_return_{h}");
        foreach (var w in DatasetConstants.RollingWindows)
        {
            parts.Add($"{Rolling("AVG", "index_price", w)} AS price_roll{w}_mean");
            parts.Add($"{Rolling("STDDEV_POP", "index_price", w)} AS price_roll{w}_std");
            parts.Add($"{Rolling("MIN", "index_price", w)} AS price_roll{w}_min");
            parts.Add($"{Rolling("MAX", "index_price", w)} AS price_roll{w}_max");
        }
        foreach (var w in DatasetConstants.RollingWindows)
            parts.Add(
                $"(index_price::double precision / NULLIF({Rolling("AVG", "index_price", w)}, 0))::double precision " +
                $"AS price_to_roll{w}_mean");
        foreach (var w in DatasetConstants.RollingWindows)
            parts.Add(
                $"({Rolling("STDDEV_POP", "index_price", w)} / NULLIF({Rolling("AVG", "index_price", w)}, 0))::double precision " +
                $"AS price_vol_{w}");
        foreach (var w in DatasetConstants.RollingWindows)
        ...
        // ── Candle shape ─────────────────────────────────────────────────
        parts.Add(
            $"ABS(index_price::double precision - open_price::double precision)::double precision " +
            $"AS candle_body");
        parts.Add(
            $"(high_price::double precision - GREATEST(index_price::double precision, open_price::double precision))::double precision " +
            $"AS upper_wick");
        parts.Add(
            $"(LEAST(index_price::double precision, open_price::double precision) - low_price::double precision)::double precision " +
            $"AS lower_wick");
            ...
            WITH tr_prep AS (
                SELECT
                    ...
                    GREATEST(
                        ...
                        ABS(high_price::double precision - LAG(index_price::double precision, 1) OVER w),
                        ABS(low_price::double precision  - LAG(index_price::double precision, 1) OVER w)
        ...
        var affected = await conn.ExecuteAsync(new CommandDefinition(updateSql, cancellationToken: ct));
        return affected;
    }
}
