...
| Файл | Ключевые объекты | Описание |
| ---- | ---------------- | -------- |
| `BybitApiClient.cs` | `BybitApiClient` | HTTP-клиент к Bybit REST v5 (`/linear`). `GetJsonAsync` реализует полную политику retry: до `MaxRetries=8` попыток; HTTP 429 → `max(Retry-After, 5)`s; `retCode 10006` → 5–10 s jitter; `retCode 10018` → 30 s; HTTP 5xx → экспоненциальный backoff; `IsTransient()` ловит `HttpRequestException`, `TaskCanceledException{inner:TimeoutException}`, `SocketException`, `IOException{inner:SocketException}`. HTTP-таймаут `90 с`. Все time-series методы нарезают диапазон на окна и запускают через `Task.WhenAll` + `SemaphoreSlim(maxParallel)`; для heavy TF (`1m`/`3m`) `maxParallel=2`, для остальных `maxParallel=8`. Методы: `FetchInstrumentDetailsAsync`, `FetchKlinesAsync`, `FetchFundingRatesAsync`, `FetchOpenInterestAsync`. |
...
| `Markets/BinanceApiClient.cs` | `BinanceApiClient` | HTTP-клиент к Binance USDT-margined futures: `/fapi/v1/exchangeInfo`, `/fapi/v1/klines`, `/fapi/v1/fundingRate`, `/futures/data/openInterestHist`. Нормализует dataset TF → Binance interval/period, режет диапазоны на окна, дедуплицирует по timestamp и возвращает те же tuple-shape'ы, что Bybit. |
| `Markets/KrakenApiClient.cs` | `KrakenApiClient` | HTTP-клиент к Kraken public market data: `/0/public/AssetPairs` для symbol resolution и `/0/public/OHLC` для свечей. Funding/OI не отдаёт (возвращает пустые серии), а неподдерживаемые TF (`3m`, `120m`, `360m`, `720m`) агрегирует из меньших upstream-баров. Ограничение upstream: API доступен только по последним 720 base candles. |
