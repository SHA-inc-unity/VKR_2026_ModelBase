...
| `docker-compose.yml` | Сервисы: `postgres` (порт 5433 host), `data` (порт 8100). Подключается к `modelline_net` (external). Env `MINIO_PUBLIC_URL` (default `http://localhost:9000`) — standalone/dev fallback для browser downloads. В proxy-контуре предпочтителен внешний host root (напр. `https://sha-trade.tech`) с тем же signed bucket path `/modelline-blobs/...`; внутренний `MINIO_ENDPOINT=http://minio:9000` с хоста не резолвится. |
