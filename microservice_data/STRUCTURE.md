...
Точка входа: регистрация DI, Serilog, Kafka consumer, job-runner, MinIO, healthchecks и роутинга. Важный lifecycle-инвариант: `Program` **не** ждёт `EnsureSchemaAsync()` до `app.Run()`; HTTP `/health` и `/ready` поднимаются сразу, а bootstrap jobs-схемы уходит в `DatasetJobRunner`.
...
| `KafkaConsumerService.Jobs.cs` | partial `KafkaConsumerService` | Handlers для `cmd.data.dataset.jobs.{start,cancel,get,list}`. **`HandleJobsStartAsync` всегда отвечает** — сначала precheck `_jobsRepo.SchemaReady` (→ `{error, code:"schema_not_ready"}`), затем валидация (`bad_request`) и отдельный ingest-precheck по `params.exchange`: пока поддержан только `bybit`, поэтому `binance`/`kraken` сразу получают `{ error, code:"unsupported_exchange" }` до постановки job в очередь. После этого вызывается `StartAsync`. Все исключения (`ArgumentException`, `InvalidOperationException`, `Npgsql.PostgresException`, `NpgsqlException`, generic) ловятся и возвращаются как `{error, code}` (`pg_<SQLSTATE>` для SQL-ошибок, `db_unavailable` для connection-level DB failure, `internal_error` иначе) — никогда не уходит наружу как unhandled exception, поэтому admin-UI получает явный отказ вместо 10-секундного Kafka-таймаута. **Авто-вычисление `target_table` для ingest**: если `target_table` не пришёл в payload, но `type=='ingest'` и заданы `target_symbol`/`target_timeframe`, handler вызывает `DatasetCore.MakeTableName(symbol, timeframe)` ДО хеширования params — иначе `JobLockManager` свернул бы все ALL-mode job'ы в один global key `external_io::*` и они исполнялись бы строго последовательно, что внешне выглядит как «застряли в queued». `params_hash` считается уже от заполненного `target_table`, поэтому dedup продолжает корректно работать. |
