...
| `Markets/BinanceApiClient.cs` | `BinanceApiClient` | HTTP-клиент к Binance USDT-margined futures: `/fapi/v1/exchangeInfo`, `/fapi/v1/klines`, `/fapi/v1/fundingRate`, `/futures/data/openInterestHist`. Нормализует dataset TF → Binance interval/period, режет диапазоны на окна, дедуплицирует по timestamp и возвращает те же tuple-shape'ы, что Bybit. JSON читается целиком под тем же `RequestTimeoutSeconds`, так что body-read больше не может зависнуть вне общего HTTP timeout. |
