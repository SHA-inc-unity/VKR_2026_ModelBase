...
Точка входа: регистрация DI, Serilog, Kafka consumer, job-runner, MinIO, healthchecks и роутинга. В DI теперь регистрируются typed `HttpClient`-ы для `BybitApiClient`, `BinanceApiClient`, `KrakenApiClient`, singleton `MarketDataClientFactory` и process-local rate limiters для `Bybit`, `Binance`, `Kraken`. Важный lifecycle-инвариант: `Program` **не** ждёт `EnsureSchemaAsync()` до `app.Run()`; HTTP `/health` и `/ready` поднимаются сразу, а bootstrap jobs-схемы уходит в `DatasetJobRunner`.

### Settings/

| Файл | Ключевые объекты | Описание |
| ---- | ---------------- | -------- |
| `DataServiceSettings.cs` | `DataServiceSettings`, `PostgresSettings`, `KafkaSettings`, `MinioSettings`, `ApiSettings` | Strongly-typed конфиг из `appsettings.json` секции `"DataService"`. Переопределяется env-переменными (`DataService__Postgres__Host` и т.д.). `ConnectionString` строится автоматически. `MinioSettings` содержит `Endpoint` (внутренний, `http://minio:9000`, используется как SDK ServiceURL и для server-to-server presigned URL — `cmd.data.dataset.export_full` → analitic) и `PublicDownloadBaseUrl` (browser-facing, по умолчанию `http://localhost:8501`, env `PUBLIC_DOWNLOAD_BASE_URL` — внешний вход infra-nginx, проксирует `/modelline-blobs/*` в MinIO; используется для browser-bound presigned URL — single/ALL CSV-export, anomaly report). |
...
| `DatasetConstants.cs` | `DatasetConstants` (static) | Таймфреймы, интервалы OI, `RawTableSchema` (13 колонок), `BuildFeatureTableSchema()` (37 фич колонок), `FullTableSchema` (50 всего), дефолты лимитов. Ключевые константы: `RequestTimeoutSeconds=90` (HTTP-таймаут), `MaxRetries=8`, `MaxParallelApiWorkers=8` (дефолт), `MaxParallelApiWorkers1m=2` (для 1m/3m), `HeavyTimeframes={"1m","3m"}`. |

### Bybit/

| Файл | Ключевые объекты | Описание |
| ---- | ---------------- | -------- |
| `BybitApiClient.cs` | `BybitApiClient` | HTTP-клиент к Bybit REST v5 (`/linear`). `GetJsonAsync` реализует полную политику retry: до `MaxRetries=8` попыток; HTTP 429 → `max(Retry-After, 5)`s; `retCode 10006` → 5–10 s jitter; `retCode 10018` → 30 s; HTTP 5xx → экспоненциальный backoff; `IsTransient()` ловит `HttpRequestException`, `TaskCanceledException{inner:TimeoutException}`, `SocketException`, `IOException{inner:SocketException}`. HTTP-таймаут `90 с`, причём теперь он покрывает чтение всего response body, а не только headers. Все time-series методы нарезают диапазон на окна и запускают через `Task.WhenAll` + `SemaphoreSlim(maxParallel)`; для heavy TF (`1m`/`3m`) `maxParallel=2`, для остальных `maxParallel=8`. Методы: `FetchInstrumentDetailsAsync`, `FetchKlinesAsync`, `FetchFundingRatesAsync`, `FetchOpenInterestAsync`. |

### Markets/

| Файл | Ключевые объекты | Описание |
| ---- | ---------------- | -------- |
| `Markets/IMarketDataClient.cs` | `IMarketDataClient` | Общий контракт exchange-клиента: `FetchInstrumentDetailsAsync`, `FetchKlinesAsync`, `FetchFundingRatesAsync`, `FetchOpenInterestAsync`. Используется и legacy Kafka-ingest path, и job handlers. |
