...
| **Background lazy hydrate** | For a known symbol/timeframe with a missing local window, `ChartService` schedules queued ingest via `cmd.data.dataset.jobs.start`/`get` in the background and immediately returns `status="pending"`. It does not keep the HTTP request open waiting for ingest completion. |
...
| **Ingest error cooldown** | If a background ingest fails, the ingest lock is replaced with a short error cooldown. During that window `GET /api/v1/market/chart` now returns `503 SERVICE_BUSY` instead of another false `pending`. |
