...
| **Sync lazy hydrate** | For a known symbol/timeframe with a missing or incomplete local window, `ChartService` first waits for `cmd.data.dataset.ingest` on the requested window and re-reads rows in the same HTTP request. `pending` is now a fallback state when another request already owns the ingest lock or the hydrate attempt fails. |
