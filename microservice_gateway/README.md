...
| **Bounded sync hydrate** | For a known symbol/timeframe with a missing or incomplete local window, `ChartService` acquires the ingest lock, runs queued ingest via `cmd.data.dataset.jobs.start`/`get`, then rereads rows inside the same HTTP request. If candles become visible in budget, the route returns `status="ok"` or `status="partial"`; `pending` is reserved for an already-held ingest lock, a still-running ingest after the wait budget, or `claim_check`. |
| **Per-table ingest serialization** | Because the hydrate path goes through `dataset_jobs`, two concurrent chart hydrates for the same dataset table cannot execute together: `DatasetJobRunner` keeps the global ingest cap at 4 and serializes active jobs by `target_table`. |
| **Ingest error cooldown** | If a hydrate attempt fails, the ingest lock is replaced with a short error cooldown. During that window `GET /api/v1/market/chart` returns `503 SERVICE_BUSY` instead of another false `pending`. |
| **Explicit rows failure split** | `DataServiceClient` now distinguishes empty rows, `claim_check`, and explicit downstream `latest_rows` / `rows` failures. `ChartService` returns `pending` only for real hydrate/claim-check paths; downstream failures and timeouts surface as `503`. |
