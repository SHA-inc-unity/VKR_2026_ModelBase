...
| `Controllers/` | HTTP endpoints gateway; `DashboardController` работает в optional-auth режиме (guest получает public dashboard, user — personal sections), а `AdminController` помечен `DisableCors`, потому что `/api/admin/*` рассчитан на server-to-server admin facade, а не на browser JS. В admin facade теперь есть и dedicated `market-watcher/*` routes для split-mode страницы `/market-watcher` в `microservice_admin`. |
