...
| Файл | Описание |
| ---- | -------- |
| `docker-compose.yml` | Запускает: Redpanda, Redpanda Console, MinIO, MinIO Console + init и **nginx** (всегда, без profile-флагов) — ingress/download endpoint backend/full-стека на host-порте 8501 (override через `NGINX_PORT`). `nginx` проксирует `/admin/*` → `admin:3000` и `/modelline-blobs/*` → `minio:9000`. Данные Redpanda и MinIO хранятся в repo-local bind mounts `../.runtime-data/microservice_infra/{redpanda,minio}`. |
