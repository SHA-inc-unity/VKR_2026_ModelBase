...
| Файл | Описание |
| ---- | -------- |
| `docker-compose.yml` | Запускает: Redpanda, Redpanda Console, MinIO, MinIO Console + init, one-shot `nginx-cert-init` и **nginx** (всегда, без profile-флагов) — ingress/download endpoint backend/full-стека на host-порте 8501 (override через `NGINX_PORT`) и HTTPS admin facade на `ADMIN_BACKEND_PORT` (default `8443`). `nginx` проксирует `/admin/*` → `admin:3000` и `/modelline-blobs/*` → `minio:9000`. Если в `ADMIN_BACKEND_CERTS_DIR` нет `tls.crt/tls.key`, `nginx-cert-init` автоматически генерирует self-signed cert до старта nginx. Данные Redpanda и MinIO хранятся в repo-local bind mounts `../.runtime-data/microservice_infra/{redpanda,minio}`. В split deployment Redpanda external advertise address и host-порты configurable через `REDPANDA_EXTERNAL_HOST`, `REDPANDA_EXTERNAL_PORT`, `REDPANDA_ADMIN_PORT`, `REDPANDA_CONSOLE_PORT`, `MINIO_API_PORT`, `MINIO_CONSOLE_PORT`. |
| `nginx/nginx.conf` | Конфиг nginx: `default_server`, `/admin/*` → admin:3000 (включая `/admin/api/events` SSE), `/modelline-blobs/*` → minio:9000 без буферизации (`proxy_buffering off`, `proxy_request_buffering off`, `proxy_read_timeout 3600s`, `client_max_body_size 0` — для многогигабайтных CSV/ZIP экспортов). Split TLS facade на `:8443` проксирует `/api/admin/*` → `gateway-service:5020`, `/modelline-blobs/*` → `minio:9000`, держит отдельный `GET /health` для facade liveness и публикует `GET /health/ready` как proxy в `gateway-service:5020/health/ready`. В `noadmin` deployment этот nginx остаётся download ingress-ом backend-хоста, а отдельная remote admin-head не обязана ходить через локальный `/admin/*`. |
...
| `.env.example` | Базовые infra/env defaults: `MINIO_ROOT_USER`, `MINIO_ROOT_PASSWORD`, `REDPANDA_EXTERNAL_HOST`, `REDPANDA_EXTERNAL_PORT`, `REDPANDA_ADMIN_PORT`, `REDPANDA_CONSOLE_PORT`, `MINIO_API_PORT`, `MINIO_CONSOLE_PORT`, `NGINX_PORT`, `ADMIN_BACKEND_PORT`, `ADMIN_BACKEND_CERTS_DIR`. Комментарии фиксируют, что пустой cert-dir теперь автоматически заполняется self-signed парой на первом старте. |
...
| `redpanda-janitor` | `redpandadata/redpanda` | — | 6-часовой sweep осиротевших `reply.*` топиков (только пустые, с HW=0); живые long-lived owners seed-ят startup marker, поэтому их inbox'ы не остаются на `HW=0` и не должны попадать под sweep |
...
| `nginx-cert-init` | build (`nginx/cert-init.Dockerfile`) | — | One-shot TLS bootstrap: если в `ADMIN_BACKEND_CERTS_DIR` нет `tls.crt/tls.key`, генерирует self-signed cert для backend facade `:8443`. Existing cert files не перетирает. |
| `nginx` | `nginx:1.27-alpine` | host `8501` → container `80`, host `8443` → container `8443` | Browser-facing ingress local/full стека и download ingress backend-host'а в split deployment: `/admin/*` → admin:3000 (если локальный admin поднят), `/modelline-blobs/*` → minio:9000 без ломания signed path/query. Параллельно держит TLS admin facade на `ADMIN_BACKEND_PORT` (default `8443`), где `/api/admin/*` проксируется в `gateway-service:5020`; стартует только после успешного `nginx-cert-init`. |
