...
| `vpn/server-entrypoint.sh` | Entrypoint для compose-сервиса `vpn` (profile `vpn`): генерирует WireGuard ключи, пишет `wg0-server.conf` + `client.conf`, затем поднимает интерфейс `wg0` (`10.44.0.1/24`) и пишет `.ready` маркер. Перед `wg setconf` прогоняет `wg0-server.conf` через `wg-quick strip`, чтобы wg-quick-поля (`Address`) не ломали wireguard-native apply path. Ожидает, что compose уже подтянул `wireguard-tools`, `iproute2-minimal`, `kmod`. |
