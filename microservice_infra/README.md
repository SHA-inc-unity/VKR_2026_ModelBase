...
`docker compose up -d` больше не требует ручной подготовки `tls.crt` / `tls.key`.
One-shot контейнер `nginx-cert-init` перед стартом nginx проверяет каталог
`ADMIN_BACKEND_CERTS_DIR` и, если он пустой, автоматически генерирует туда
self-signed сертификат для `:8443`. Если в каталоге уже лежат ваши реальные
сертификаты, compose переиспользует их без перезаписи.

У этого autogenerated cert ожидаемое ограничение: он self-signed. Поэтому
`microservice_admin` (`admin-online`) по умолчанию запускается с
`ADMIN_BACKEND_TLS_INSECURE=1`, чтобы split deployment работал из коробки.
После установки доверенного сертификата на backend-host переведите admin-host
на `ADMIN_BACKEND_TLS_INSECURE=0`.
...
Для split deployment после первого старта можно сразу проверить TLS facade:
...
- Здоровые long-lived inbox owners теперь сами seed-ят новый `reply.*`
  topic bootstrap-marker'ом на старте, поэтому даже до первого реального
  reply HighWatermark уже уходит выше `0`. Это защищает живой, но idle inbox
  от удаления janitor-ом.
