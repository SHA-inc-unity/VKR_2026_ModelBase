...
Per-user notification **inbox** + **real-time SSE delivery** + a **price-drift
watcher**. ASP.NET Core 8, Clean Architecture, its own PostgreSQL. It
...
A notification has two delivery paths:
...
2. **SSE stream (ephemeral).** `GET /api/notifications/stream?access_token=<jwt>`
   ...
   real-time (the inbox still has the row on next fetch). **This is the gap that
   push notifications will eventually close.** The query-param token path exists
   because browser `EventSource` cannot set an `Authorization` header — the
   stream endpoint is `[AllowAnonymous]` and validates the token manually via
   `JwtTokenValidator`.
...
Anything new (e.g. web-push / FCM) hooks in **here**, after persist — do not add
...
- **EF auto-migrate on boot** (`MigrateAndSeedAsync`): applies pending migrations, then a `RequiredTables` whitelist (`notifications`, `notification_settings`) sanity-check — if the schema is fully empty after migrate it recreates tables from the model; a *partial* schema is a hard-fail.
