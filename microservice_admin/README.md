...
train/anomaly run) с кратким payload/response summary, duration и
`correlationId`. Источник хранится отдельно от `/api/logs`, переживает reload
страницы и при наличии Redis переживает restart admin-процесса, поэтому Queue
показывает именно историю очереди, а Logs остаётся общим операторским trace.
...
| `src/lib/queueHistoryStore.ts` | **Server-only.** Отдельный persistent ring buffer истории очереди. Хранит только крупные queue-операции, которые `/api/kafka` завершил success/error. Источник читается страницей Queue через `/api/queue/history`; при наличии Redis история переживает restart admin-process, при отсутствии Redis остаётся best-effort in-memory. |
