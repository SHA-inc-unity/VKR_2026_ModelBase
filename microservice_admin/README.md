...
Queue-history намеренно отфильтрована от ping/read-only шума: `jobs.list`,
`jobs.get`, health-check'и и прочие polling-вызовы остаются в Logs, а Queue
показывает только крупные операции вроде `jobs.start`, delete/clean/export,
repair/recompute и train/anomaly run. Для этих action-ов `/api/kafka`
сохраняет краткий payload/response summary, чтобы в очереди было видно не
только факт запроса, но и его реальный диапазон/таблицу/job id.
