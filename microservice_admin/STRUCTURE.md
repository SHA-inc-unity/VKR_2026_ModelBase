...
| `docker-compose.yml` | Поддерживает **два режима**. `admin` — local-stack service: `expose: ["3000"]`, подключён к `modelline_net`, наружу не публикуется, browser-facing вход даёт nginx из `microservice_infra` на `8501/admin/*`. `admin-online` — profile `online`, отдельная online-head нода для split deployment: по умолчанию публикует `${ADMIN_PORT:-443}:3000`, не требует `modelline_net` и читает внешние адреса из `ONLINE_*` namespace (`ONLINE_KAFKA_BOOTSTRAP_SERVERS`, `ONLINE_REDPANDA_ADMIN_URL`, `ONLINE_ACCOUNT_URL`, `ONLINE_GATEWAY_URL`, `ONLINE_MINIO_URL`, `ONLINE_REDIS_URL`). Для published backend ports default-значения online health endpoints выровнены на `7510/7520`. Launcher `microservicestarter` может сохранить `ONLINE_BACKEND_HOST` и автоматически вывести из него все `ONLINE_*` в `.env`, чтобы remote admin-head не требовал ручного редактирования шести адресов. Для profile `vpn` compose bootstrap-ит `wireguard-tools`, `iproute2-minimal`, `kmod` и `iptables`, чтобы `vpn-client` не падал на clean host из-за отсутствующего `ip`/`modprobe` и сам выставлял client-side allow-правила для `wg0`. В обоих случаях Admin не ходит в MinIO напрямую и остаётся zero-byte для dataset export. |
