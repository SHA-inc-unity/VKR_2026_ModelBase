...
| `login/page.tsx` | Login-only экран admin-панели. Форма вызывает `POST /api/auth/login`, принимает только Account Service пользователя с ролью `admin`, после успеха httpOnly cookie содержит admin JWT. Sidebar скрывается на `/login`, остальные pages/API закрываются middleware-ом и server-side проверками. |
...
| `POST /api/auth/login` / `POST /api/auth/logout` / `GET /api/auth/session` | `api/auth/*/route.ts` | Admin auth routes. Login принимает email/password, вызывает Account Service и разрешает вход только при роли `admin`; logout очищает httpOnly cookies; session проверяет текущую admin-сессию. |
