...
| `docker-compose.yml` | Поддерживает **два режима**. `admin` — local-stack service: `expose: ["3000"]`, подключён к `modelline_net`, наружу не публикуется, browser-facing вход даёт nginx из `microservice_infra` на `8501/admin/*`. `admin-online` — profile `online`, отдельная online-head нода для split deployment: по умолчанию публикует `${ADMIN_PORT:-443}:3000`, не требует `modelline_net`, читает внешние адреса из `ONLINE_*` namespace (`ONLINE_KAFKA_BOOTSTRAP_SERVERS`, `ONLINE_REDPANDA_ADMIN_URL`, `ONLINE_ACCOUNT_URL`, `ONLINE_GATEWAY_URL`, `ONLINE_MINIO_URL`, `ONLINE_REDIS_URL`) и прокидывает в контейнер `ADMIN_BACKEND_BASE_URL` + `ADMIN_BACKEND_SHARED_TOKEN` для нового HTTP facade path. Для published backend ports default-значения online health endpoints выровнены на `7510/7520`. Launcher `microservicestarter` может сохранить `ONLINE_BACKEND_HOST`, автоматически вывести из него все `ONLINE_*`, предложить `ADMIN_BACKEND_BASE_URL` и спросить недостающий `ADMIN_BACKEND_SHARED_TOKEN` в `.env`, чтобы remote admin-head не требовал ручного редактирования split-конфига. В обоих случаях Admin не ходит в MinIO напрямую и остаётся zero-byte для dataset export. |
| `docker-compose.yml` | Поддерживает **два режима**. `admin` — local-stack service: `expose: ["3000"]`, подключён к `modelline_net`, наружу не публикуется, browser-facing вход даёт nginx из `microservice_infra` на `8501/admin/*`. `admin-online` — profile `online`, отдельная online-head нода для split deployment: по умолчанию публикует `${ADMIN_PORT:-443}:3000`, не требует `modelline_net` и читает внешние адреса из `ONLINE_*` namespace (`ONLINE_KAFKA_BOOTSTRAP_SERVERS`, `ONLINE_REDPANDA_ADMIN_URL`, `ONLINE_ACCOUNT_URL`, `ONLINE_GATEWAY_URL`, `ONLINE_MINIO_URL`, `ONLINE_REDIS_URL`). Для published backend ports default-значения online health endpoints выровнены на `7510/7520`. Launcher `microservicestarter` может сохранить `ONLINE_BACKEND_HOST` и автоматически вывести из него все `ONLINE_*` в `.env`, чтобы remote admin-head не требовал ручного редактирования шести адресов. Для profile `vpn` compose bootstrap-ит `wireguard-tools`, `iproute2-minimal`, `kmod` и `iptables`; рядом поднимается `wstunnel-client`, который слушает локальный UDP `51820` и соединяется с backend `ws://VPN_SERVER_URL:8443`. В обоих случаях Admin не ходит в MinIO напрямую и остаётся zero-byte для dataset export. |
| `vpn/client-entrypoint.sh` | Entrypoint для compose-сервиса `vpn-client` (profile `vpn`): читает `/vpn/state/wg0.conf` (написан launcher из join token), извлекает `Address`, прогоняет конфиг через `wg-quick strip`, поднимает WireGuard интерфейс `wg0` (`10.44.0.2/32`), затем явно ставит route-ы из `AllowedIPs` на `wg0`, добавляет idempotent `iptables` allow для `wg0` в `INPUT` и `DOCKER-USER`, после чего пишет `.ready` маркер. Это нужно, потому что `wg setconf` сам route-ы не создаёт, а без них `10.44.0.1:*` уходит в default route хоста даже при успешном handshake; отдельно host firewall может отрезать уже ответный трафик backend -> admin. Автоматически запускается launcher при `onlyadmin <JOIN_TOKEN>` или при `onlyadmin` с существующим `wg0.conf`. Ожидает, что compose уже подтянул `wireguard-tools`, `iproute2-minimal`, `kmod`, `iptables`. |
| `vpn/client-entrypoint.sh` | Entrypoint для compose-сервиса `vpn-client` (profile `vpn`): читает `/vpn/state/wg0.conf` (написан launcher из join token), поднимает WireGuard интерфейс `wg0` (`10.44.0.2/32`), пишет `.ready` маркер. Автоматически запускается launcher при `onlyadmin <JOIN_TOKEN>` или при `onlyadmin` с существующим `wg0.conf`. |
...
| Маршрут | Файл | Описание |
| ------- | ---- | -------- |
| `POST /api/kafka` | `api/kafka/route.ts` | Универсальный backend proxy. Body: `{ topic, payload?, timeoutMs? }`. **Local mode**: маршрутизирует в Kafka через `lib/kafka.ts`. **Split mode** (`ADMIN_BACKEND_BASE_URL`): вызывает `backendCall()` через `lib/backendClient.ts`. Read-only топики коалесцируются в обоих режимах. Возвращает `{ data }` или `{ error }`. |
...
| `GET /api/health` | `api/health/route.ts` | Параллельный HTTP health-probe (через `Promise.allSettled`) для четырёх server-side адресов: `REDPANDA_ADMIN_URL/v1/status/ready`, `MINIO_URL/minio/health/live`, `ACCOUNT_URL/health`, `GATEWAY_URL/health`. Дополнительно route делает server-side Kafka connectivity probe через `lib/kafka.ts` и возвращает broker-level `kafka.status`, `bootstrapServers`, `replyInbox?`, `error?`. Таймаут HTTP probe — `2 000 мс` (`AbortSignal.timeout`). Local-stack использует docker-hostname, online-head — те же поля, но заполненные из `ONLINE_*` namespace на уровне compose (`admin-online`). В ответ дополнительно включается `connectionTarget` из runtime env `BACKEND_CONNECTION_TARGET`: local stack жёстко отдаёт `localhost`, `admin-online` — `ONLINE_BACKEND_HOST`, чтобы dashboard мог явно показать текущий backend-host. Sidebar использует именно этот broker-level статус, а не `cmd.data.db.ping`, поэтому `Kafka error` теперь значит реальную недоступность Kafka path, а не косвенный отказ data-service reply. `dynamic = 'force-dynamic'` |
