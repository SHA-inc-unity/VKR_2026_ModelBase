...
| `login/page.tsx` | Login-only экран admin-панели. Форма вызывает `POST /api/auth/login`, принимает только Account Service пользователя с ролью `admin`, после успеха httpOnly cookies содержат access/refresh tokens. Sidebar скрывается на `/login`, остальные pages/API закрываются middleware-ом и server-side проверками; если admin-сессия уже есть или может быть восстановлена по refresh token, middleware не даёт форме отрисоваться и редиректит в панель. |
...
| `adminSession.ts` | `loginAdmin()`, `requireAdminSession()`, cookie helpers | **Server-only.** Auth bridge admin-панели: login ходит в Account Service `/api/account/login`, принимает только роль `admin`, кладёт access/refresh token в httpOnly cookies; protected API routes используют `requireAdminSession()` и в local direct-Kafka mode дополнительно проверяют access token через `/api/account/me`. Silent refresh access token при истечении выполняет `src/middleware.ts` до попадания запроса в route/page, используя тот же refresh cookie. |
...
| `POST /api/auth/login` / `POST /api/auth/logout` / `GET /api/auth/session` | `api/auth/*/route.ts` | Admin auth routes. Login принимает email/password, вызывает Account Service и разрешает вход только при роли `admin`; logout очищает httpOnly cookies; session проверяет текущую admin-сессию. Сам `GET /api/auth/session` больше не относится к anonymous-public path: он проходит через тот же middleware/session flow, поэтому cached login может быть silently восстановлен по refresh token до ответа route-а. |
