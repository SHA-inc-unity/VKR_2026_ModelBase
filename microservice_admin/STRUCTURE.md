...
- `src/hooks/useDatasetJobs.ts` — process-local store на
  ...
  `failed/canceled` — через 10 с. Terminal completions best-effort
  записываются в `/api/queue/history`, поэтому Queue сохраняет след
  завершения даже после автоскрытия live-card.
...
- `src/app/download/page.tsx` — использует `useDatasetJobsFeed()` для
  ...
  **Honest job lifecycle** (Step 4):
  ...
  - `handleIngest` *первым делом* `await refreshCoverageState()`
    ...
    `Symbol=ALL` и `Exchange=ALL` не создают special backend contract:
    admin сам fan-out'ит aggregate selection в cartesian product
    `EXCHANGES × SYMBOLS × TIMEFRAMES` по мере необходимости,
    хранит tracked jobs по ключу `exchange::symbol::timeframe` и при
    `Symbol=ALL` принудительно переводит UI в `timeframe=ALL`.
  ...
  - `AllIngestProgress` больше не считает «прогресс по числу TF`.
    ...
    detail, elapsed и short job id; label теперь может быть как просто
    TF, так и full scope `Binance BTCUSDT 1m` для multi-dataset fan-out. Успешный terminal-state с
    `completed=0` показывается как нормальный no-op (`без новых строк`
    / `дозагрузка не потребовалась`). Failed ALL-mode TF теперь
    автоскрываются через 10 с и одновременно пишут ошибку в local
    `useHistory()` Action History, чтобы не распирать layout бесконечными
    error rows. Polling-fallback `hasActive`
...
| `src/middleware.ts` | Global auth gate admin-панели. Пропускает только `/login`, `/api/auth/login`, `/api/auth/logout`, `/api/health` и static assets без обязательной admin-сессии. Для остальных pages/API сначала проверяет admin access token, а если он истёк, но refresh token ещё жив, делает silent refresh через Account Service, обновляет cookies и подменяет request `cookie` header для downstream route/page. При уже живой или восстановимой сессии `/login` редиректится назад на dashboard. |
...
| `page.tsx` | Dashboard Bento Grid. В header рядом с title теперь есть компактный monospace badge с `connectionTarget`, refresh button и exchange dropdown для dataset widgets. Ниже остаётся отдельный заметный `Card`: `Connected to` + `connectionTarget` (host/IP и режимный badge `localhost` / `admin-online`). Значение приходит из `/api/health` и в split deployment даёт оператору мгновенную проверку, что admin-head смотрит на правильный backend-host, а не в локальный `localhost`. В connection-card показываются `Kafka bootstrap` и broker error, если `KAFKA_BOOTSTRAP_SERVERS` недоступен. Row 1: `StatCard` (×4) с `border-l-4` акцентами (`grid-cols-2 xl:grid-cols-4`); dataset-related cards теперь считают только непустые таблицы выбранной биржи. Row 2: `grid-cols-1 lg:grid-cols-2` — стек из 6 `ServiceCard` слева: 2 Kafka health (`microservice_data`, `microservice_analitic`) и 4 HTTP probe через `fetchInfraHealth()` (`Redpanda`, `MinIO`, `account`, `gateway`), `CoverageBar` chart справа. Coverage chart теперь получает только non-zero tables выбранной биржи, сортируется по coverage и поднимает height динамически, чтобы при большем числе строк не схлопываться в низкий блок. Маппинг infra → `ServiceHealth`: `online` → `{ status: 'ok' }`, `offline` → `{ status: 'error', error }`. В online-head режиме те же карточки продолжают работать через env-переопределения `ONLINE_*`, без локальной `modelline_net`. `anyLoading` = `dataLoading` + `tablesLoading` + `modelsLoading` + `infraLoading` (OR). Row 3: Dataset shadcn Table, тоже отфильтрованная по selected exchange и без `0 rows`. Авто-рефреш через `useEvents(EVT_ANALYTICS_MODEL_READY)`. Empty-state placeholders в `StatCard.value` и `<span>` ячейках coverage-таблицы — `'–'` (en-dash, U+2013); JSX-комментарии — обычный `-`. **SQLite-backed cache**: на маунте читает `modelline:dashboard:v1` (TTL 60 мин) → восстанавливает `tables/coverage/modelCount` до завершения `refresh()`; после загрузки таблиц — `cacheWrite` fire-and-forget. Health-состояния не кешируются. |
| `login/page.tsx` | Login-only экран admin-панели. Форма вызывает `POST /api/auth/login`, принимает только Account Service пользователя с ролью `admin`, после успеха httpOnly cookies содержат access/refresh tokens. Поле логина принимает username или email; на первом запуске без явного bootstrap можно войти как `admin/admin`. Sidebar скрывается на `/login`, остальные pages/API закрываются middleware-ом и server-side проверками; если admin-сессия уже есть или может быть восстановлена по refresh token, middleware не даёт форме отрисоваться и редиректит в панель. |

### Страницы

| Маршрут | Файл | Описание |
| ------- | ---- | -------- |
| `/download` | `download/page.tsx` | **Dataset страница.** Layout: `grid-cols-1 lg:grid-cols-[380px,1fr]`. Левая колонка: конфигурация, кнопки операций и ingest progress UI; правая: coverage/stat cards; ниже Available Tables, Quality Block и Action History. Ingest-button стоит в одной строке с exchange selector (`all`, `bybit`, `binance`, `kraken`), а выбранные `symbol/timeframe/dateFrom/dateTo/exchange` зеркалятся из `localStorage` в server-side SQLite state (`modelline:params:dataset`), поэтому переживают restart admin-контейнера. `Symbol=ALL` и `Exchange=ALL` означают aggregate ingest orchestration: admin fan-out'ит выбор в набор конкретных jobs по full scope `exchange::symbol::timeframe`, а single-table действия `coverage/export/repair/delete` в этих режимах отключены. `Symbol=ALL` по-прежнему фиксирует UI на `timeframe=ALL`; `Exchange=ALL` можно использовать и с конкретным timeframe. `handleIngest` больше не делает долгий ingest RPC: single-TF и `ALL` сначала `await refreshCoverageState()`, затем отправляют быстрый `CMD_DATA_DATASET_JOBS_START` (`timeoutMs: 5_000`) и переводят TF в `queued`. `running` приходит из remote job state (`useDatasetJobs`): SSE остаётся primary-каналом, а для tracked ingest jobs добавлен fallback polling `cmd.data.dataset.jobs.list` + `cmd.data.dataset.jobs.get` раз в 5 с, чтобы UI не застревал в ложном `queued/stalled`, если split admin пропустил progress/completed event. Если remote job реально создана, `loadingIngest` и page-level lock удерживаются до её terminal-state; если старт не состоялся, локальный busy-state снимается сразу по явному отказу backend. Для `ALL` локальный `AllIngestProgress` строится вокруг `allIngestJobIds` + `DatasetJobView[]`: рендерит 4 execution slot-а, очередь queued jobs, stalled-banner если очередь не двигается, и recent done/error list; running-slot показывает stage/progress/detail/elapsed/short job id, а succeeded+`completed=0` читается как нормальный no-op. Детальный `DatasetJobsPanel` из Download убран и вынесен на `/queue`, чтобы dataset-операторский экран не смешивал общую live-очередь со scoped ingest-виджетом. Ошибки старта (`schema_not_ready`, `bad_request`, `unsupported_exchange`, `db_unavailable`, `pg_*`, `internal_error`) сразу переводят конкретный TF в `error` без ложного running. Backend теперь реально исполняет ingest для `bybit`, `binance` и `kraken`, а canonical table naming стал exchange-aware: legacy Bybit остаётся `{symbol}_{timeframe}`, остальные биржи пишутся в `{exchange}_{symbol}_{timeframe}`. Из-за этого `refreshCoverageState`, delete/export helpers, cache keys и `seedQueuedJob.target_table` тоже учитывают выбранный `exchange`, чтобы queue и coverage не смешивали строки между биржами. Coverage не обнуляется; после terminal jobs, quality-check и repair выполняется `refreshCoverageState()` и `handleListTables()`, поэтому UI всегда опирается на последнее реальное состояние БД. Dataset export остаётся Kafka-only и zero-byte для Admin: `/api/export/csv` возвращает только `presigned_url` без host-нормализации и без legacy raw-localhost fallback'а — URL уже подписан data-сервисом на browser-facing origin (внешний вход infra-nginx, по умолчанию `http://localhost:8501`), а `/modelline-blobs/*` стримит файл напрямую из MinIO; `timeframe=ALL` теперь собирает ZIP по exchange-aware table names. Остальные операции сохраняют Kafka-only ownership: admin только запускает команды и отображает удалённый прогресс/результат. Repair path (`load_ohlcv`, `recompute_features`) теперь тоже exchange-aware: `download/page.tsx` больше не режет `binance`/`kraken`, `cmd.analitic.dataset.load_ohlcv` делегирует exchange-specific fetch в `cmd.data.dataset.repair_ohlcv`, а SSE topic `events.analitic.dataset.repair.progress` остаётся прежним для всех трёх бирж. |
...
| `/queue` | `queue/page.tsx` | **Operator Queue.** Седьмой пункт левого меню. Страница подключает `useDatasetJobsFeed(1500)` и совмещает `DatasetJobsPanel` с отдельной **queue history** из `GET /api/queue/history?limit=250`, а не с общим runtime log stream. Верхние counters показывают active/running/queued/finished dataset jobs и число queue-request errors; ниже идут live dataset jobs и таблица крупных операций (`jobs.start/cancel`, delete/clean/export, repair/recompute, train/anomaly run) плюс terminal dataset-job completions, с duration, payload/response summary, exchange и correlation id. `Duration` для terminal dataset jobs считается как `started_at → finished_at`, а не как latency admin request-а. Legacy `market_watch` completions и dedicated watcher control/log topics сюда специально не попадают. История читается из SQLite-backed store через `/api/queue/history`, поэтому completed queue-операции переживают restart admin-контейнера даже после автоскрытия live-card. Ping/health и polling `jobs.list/get` намеренно исключены и остаются только на `/logs`. `Clear` очищает только queue history, но не вмешивается в фактические jobs владельца-сервиса и не трогает общий runtime log buffer. |
| `/market-watcher` | `market-watcher/page.tsx` | **Dedicated Market Watcher panel.** Восьмой пункт левого меню. Client page не использует `/api/logs` и не трогает queue history: она опрашивает только `cmd.data.market_watcher.status`, `cmd.data.market_watcher.set_enabled`, `cmd.data.market_watcher.rows`, `cmd.data.market_watcher.logs`. В одном экране даёт operator control (enable/disable), runtime status (`desired/effective`, heartbeat, flush, tick, last error), paged realtime price table по tracked symbols из dataset whitelist-а и watcher-only log table. `lag_ms` показывается и по строкам, и агрегированно (avg/max lag cards). |
...
| `backendClient.ts` | `backendCall()`, `BackendClientError`, `isSplitMode`, `ADMIN_BACKEND_BASE_URL`, `ADMIN_BACKEND_TLS_INSECURE` | **Server-only. HTTP-клиент для gateway admin facade (split deployment).** Когда `ADMIN_BACKEND_BASE_URL` задан, маршрутизирует все Kafka-топики в `POST /api/admin/<path>` на backend-хосте вместо прямого Kafka. В `Authorization: Bearer` передаётся admin JWT из httpOnly cookie, а не static key. Если `ADMIN_BACKEND_TLS_INSECURE=1` и backend URL использует `https://`, модуль выставляет `NODE_TLS_REJECT_UNAUTHORIZED=0`, чтобы Next runtime принимал autogenerated self-signed cert backend facade. Содержит полную таблицу `TOPIC_PATH` (48 топиков), включая dedicated watcher surface `POST /api/admin/market-watcher/{status,set-enabled,rows,logs}` для страницы `/market-watcher`. `BackendClientError` несёт HTTP-статус, `code`, `detail` и `correlationId`; отдельно классифицирует missing admin session, `admin_backend_tls_untrusted`, `502` upstream, `503` backend Kafka unavailability, `504` timeout и network failures с причиной из `err.cause`. Runtime diagnostics идут в container logs тегом `[admin-backend]` без печати bearer token. |
| `adminSession.ts` | `loginAdmin()`, `requireAdminSession()`, cookie helpers | **Server-only.** Auth bridge admin-панели: login ходит в Account Service `/api/account/login`, принимает только роль `admin`, кладёт access/refresh token в httpOnly cookies; login identifier может быть username или email. Protected API routes используют `requireAdminSession()` и в local direct-Kafka mode дополнительно проверяют access token через `/api/account/me`. Silent refresh access token при истечении выполняет `src/middleware.ts` до попадания запроса в route/page, используя тот же refresh cookie. |
...
| `adminRuntimeLog.ts` | `writeAdminRuntimeLog()`, `readAdminRuntimeLogs()`, `clearAdminRuntimeLogs()` | **Server-only.** Process-local ring buffer последних admin runtime diagnostics. Пишется из `/api/health`, `/api/kafka` и `backendClient.ts` параллельно с console logs. Максимум 500 записей, последние читаются `/api/logs`; bearer token и payload не сохраняются. |
| `queueHistoryStore.ts` | `appendQueueHistory()`, `readQueueHistory()`, `clearQueueHistory()` | **Server-only.** Отдельный queue-history store для страницы `/queue`. Хранит success/error записи по allowlist queue-topic'ов и terminal dataset-job completions, сохраняет `topic`, `durationMs`, `payloadSummary`, `responseSummary`, `code/detail`, `correlationId` и делегирует persistence в `sqliteStore.ts`, поэтому история переживает restart admin-контейнера, пока жив `SQLITE_DB_PATH`. Dedicated watcher topics и legacy `market_watch` completions туда больше не попадают. |
...
| `topics.ts` | `Topics`, `replyInbox()` | Константы топиков Kafka. Аномалийные: `CMD_DATA_DATASET_COLUMN_STATS`, `CMD_DATA_DATASET_COLUMN_HISTOGRAM`, `CMD_DATA_DATASET_BROWSE`, `CMD_DATA_DATASET_DETECT_ANOMALIES`, `CMD_DATA_DATASET_CLEAN_PREVIEW`, `CMD_DATA_DATASET_CLEAN_APPLY`, `CMD_DATA_DATASET_AUDIT_LOG`. Отдельный control-plane market watcher-а: `CMD_DATA_MARKET_WATCHER_STATUS`, `CMD_DATA_MARKET_WATCHER_SET_ENABLED`, `CMD_DATA_MARKET_WATCHER_ROWS`, `CMD_DATA_MARKET_WATCHER_LOGS`. Сессионные/ML/диагностика: `CMD_ANALITIC_DATASET_LOAD`, `CMD_ANALITIC_DATASET_UNLOAD`, `CMD_ANALITIC_DATASET_STATUS`, `CMD_ANALITIC_ANOMALY_DBSCAN`, `CMD_ANALITIC_ANOMALY_ISOLATION_FOREST`, `CMD_ANALITIC_DATASET_DISTRIBUTION`. |
...
| Маршрут | Файл | Описание |
| ------- | ---- | -------- |
| `POST /api/kafka` | `api/kafka/route.ts` | Универсальный backend proxy. Body: `{ topic, payload?, timeoutMs? }`. Требует admin session. **Local mode**: проверяет JWT через Account Service и маршрутизирует в Kafka через `lib/kafka.ts`. **Split mode** (`ADMIN_BACKEND_BASE_URL`): вызывает `backendCall()` через `lib/backendClient.ts`, передавая admin JWT в backend facade. Read-only топики коалесцируются в обоих режимах. Возвращает `{ data }` или `{ error, status?, code?, detail?, correlationId? }`, чтобы UI видел конкретную причину split-facade отказа. Runtime diagnostics идут в container logs тегом `[api/kafka]`: branch, topic, timeout, coalesce, duration, code и correlationId. |
| `POST /api/auth/login` / `POST /api/auth/logout` / `GET /api/auth/session` | `api/auth/*/route.ts` | Admin auth routes. Login принимает `login/password` (или legacy `email/password`), вызывает Account Service и разрешает вход только при роли `admin`; logout очищает httpOnly cookies; session проверяет текущую admin-сессию. Сам `GET /api/auth/session` больше не относится к anonymous-public path: он проходит через тот же middleware/session flow, поэтому cached login может быть silently восстановлен по refresh token до ответа route-а. |
...
| `GET /api/queue/history` / `POST /api/queue/history` / `DELETE /api/queue/history` | `api/queue/history/route.ts` | Возвращает, дописывает или очищает отдельную queue history. `GET ?limit=N` ограничен диапазоном 1..400. `POST` используется для best-effort записи terminal dataset-job completions из client live-store; `durationMs` считается в браузере как `started_at → finished_at`, а legacy `market_watch` completions туда не пишутся. Источник не смешивается с `/api/logs`: Queue читает только этот route, а backing store — SQLite queue_history table, поэтому completed queue-операции сохраняются отдельно от общего admin runtime trace и переживают restart контейнера. |
