...
- `src/hooks/useDatasetJobsFeed.ts` — тонкая client-only синхронизация
  ...
  лёгкий polling active jobs. Используется на `/download` и `/queue`,
  чтобы live-store не зависел от присутствия верхнего jobs-panel на
  конкретной странице.
...
| `/queue` | `queue/page.tsx` | **Operator Queue.** Седьмой пункт левого меню. Страница подключает `useDatasetJobsFeed(5000)` и совмещает `DatasetJobsPanel` с **filtered queue history** из `GET /api/logs?limit=250`. Верхние counters показывают active/running/queued/finished dataset jobs и число request errors; ниже идут live dataset jobs и таблица только крупных операций (`jobs.start/cancel`, delete/clean/export, repair/recompute, train/anomaly run). Ping/health и polling `jobs.list/get` намеренно исключены и остаются только на `/logs`. `Clear` очищает process-local runtime buffer, но не вмешивается в фактические jobs владельца-сервиса. |
...
| `GET /api/health` | `api/health/route.ts` | Параллельный HTTP health-probe (через `Promise.allSettled`) для четырёх server-side адресов: `REDPANDA_ADMIN_URL/v1/status/ready`, `MINIO_URL/minio/health/live`, `ACCOUNT_URL/health`, `GATEWAY_URL/health`. Дополнительно route делает server-side Kafka connectivity probe через `lib/kafka.ts` и возвращает broker-level `kafka.status`, `bootstrapServers`, `replyInbox?`, `error?`. Таймаут HTTP probe — `2 000 мс` (`AbortSignal.timeout`). Local-stack использует docker-hostname, online-head — те же поля, но заполненные из `ONLINE_*` namespace на уровне compose (`admin-online`). В split mode route вычисляет `ADMIN_BACKEND_BASE_URL` из runtime env на каждый запрос, при `ADMIN_BACKEND_TLS_INSECURE=1` включает self-signed-friendly Node TLS mode и проверяет только `ADMIN_BACKEND_BASE_URL/health`, но по-прежнему отдаёт полный `InfraHealthResponse` с валидным `KafkaBrokerHealth` (`online/offline`, `bootstrapServers`) и согласованными service-status полями для dashboard. Runtime diagnostics идут в container logs тегом `[api/health]`: выбранная ветка, backend URL, TLS flag, probe status/error и duration. В ответ дополнительно включается `connectionTarget` из runtime env `BACKEND_CONNECTION_TARGET`: local stack жёстко отдаёт `localhost`, `admin-online` — `ONLINE_BACKEND_HOST`, чтобы dashboard мог явно показать текущий backend-host. Sidebar использует именно этот broker-level статус, а не `cmd.data.db.ping`, поэтому `Kafka error` теперь значит реальную недоступность Kafka path, а не косвенный отказ data-service reply. `dynamic = 'force-dynamic'` |
