...
| `GET /api/queue/history` / `DELETE /api/queue/history` | `api/queue/history/route.ts` | Возвращает или очищает отдельную queue history. `GET ?limit=N` ограничен диапазоном 1..400. Источник не смешивается с `/api/logs`: Queue читает только этот route, поэтому completed queue-операции сохраняются отдельно от общего admin runtime trace. |
