...
| `GET /api/queue/history` / `POST /api/queue/history` / `DELETE /api/queue/history` | `api/queue/history/route.ts` | Возвращает, дописывает или очищает отдельную queue history. `GET ?limit=N` ограничен диапазоном 1..400. `POST` используется для best-effort записи terminal dataset-job completions из client live-store. Источник не смешивается с `/api/logs`: Queue читает только этот route, а backing store — SQLite queue_history table, поэтому completed queue-операции сохраняются отдельно от общего admin runtime trace и переживают restart контейнера. |
