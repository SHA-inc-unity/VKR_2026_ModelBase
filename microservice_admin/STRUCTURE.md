...
| `/queue` | `queue/page.tsx` | **Operator Queue.** Седьмой пункт левого меню. Страница подключает `useDatasetJobsFeed(5000)` и совмещает `DatasetJobsPanel` с request-stream из `GET /api/logs?limit=250`. Верхние counters показывают active/running/queued/finished dataset jobs и число request errors; ниже идут live dataset jobs и таблица входящих admin runtime requests (`/api/health`, `/api/kafka`, `admin-backend`). `Clear` очищает только process-local runtime buffer, но не вмешивается в фактические jobs владельца-сервиса. |
