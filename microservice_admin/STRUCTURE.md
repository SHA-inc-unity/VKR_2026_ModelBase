...
- `src/app/download/page.tsx` — использует `useDatasetJobsFeed()` для
  ...
  **Honest job lifecycle** (Step 4):
  ...
  - `handleIngest` *первым делом* `await refreshCoverageState()`
    ...
    `Symbol=ALL` не создаёт special backend contract: admin сам
    fan-out'ит выбранную биржу в cartesian product `SYMBOLS × TIMEFRAMES`,
    хранит tracked jobs по ключу `symbol::timeframe` и принудительно
    переводит UI в `timeframe=ALL`.
  ...
  - `AllIngestProgress` больше не считает «прогресс по числу TF`.
    ...
    TF, так и full scope `BTCUSDT 1m` для multi-dataset fan-out. Успешный terminal-state с
