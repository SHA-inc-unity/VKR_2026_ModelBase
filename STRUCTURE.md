...
| `start.ps1` / `start.sh` | Запуск сервисов. При первом запуске создаёт `.env` и запрашивает пароль PostgreSQL. Поддерживает `core`, `noadmin`, `onlyadmin`, `full`, `scheduler`, `build`, `logs`. При `onlyadmin` поднимает отдельную online-head admin-ноду и может интерактивно дописать `ONLINE_*`, `ADMIN_BACKEND_BASE_URL`, TLS-флаг и browser-facing defaults; admin-доступ выполняется через login-only admin account, без общего ключа. В `noadmin` может спросить browser-facing backend URL, сохранить `PUBLIC_DOWNLOAD_BASE_URL` и `ADMIN_BACKEND_PORT` для `8443` facade path. В multi-service режимах сначала синхронно стартует `microservice_infra`, затем остальные сервисы уходят в параллельный fan-out. Linux-версия перед стартом создаёт `.runtime-data` bind-mount каталоги и нормализует права записи. |
...
| `restart.ps1` / `restart.sh` | `git pull` + пересборка + перезапуск. Поддерживает `core`, `noadmin`, `onlyadmin`, `full`, `deps`, `api`. При `onlyadmin` поднимает отдельную online-head admin-ноду и может интерактивно дописать `ONLINE_*`, `ADMIN_BACKEND_BASE_URL`, TLS-флаг и browser-facing defaults; admin-доступ выполняется через login-only admin account, без общего ключа. В `noadmin` может спросить browser-facing backend URL, сохранить `PUBLIC_DOWNLOAD_BASE_URL` и `ADMIN_BACKEND_PORT` для `8443` facade path. В multi-service режимах делает один `git pull`, затем синхронно поднимает `microservice_infra` и fan-out перезапускает остальные сервисы параллельно. После сборки удаляет dangling-образы. Linux-версия перед стартом создаёт `.runtime-data` bind-mount каталоги и нормализует права записи. |
