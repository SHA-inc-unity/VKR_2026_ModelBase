...
- После проверки outer UDP стало ясно, что прямой WireGuard UDP `51820` нестабилен/режется между admin-host и российским backend-host. Containerized VPN переведён на WireGuard over WebSocket/TCP `443`: backend поднимает `wstunnel-server`, admin поднимает `wstunnel-client`, join token несёт metadata для `.env`, а WireGuard client dial теперь идёт в локальный `127.0.0.1:51820`.
