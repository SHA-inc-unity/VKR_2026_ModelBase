...
def save_klines_to_sqlite(df: pd.DataFrame, symbol: str, interval: str, db_path: str | Path) -> int:
    ...
    if "start_ms" not in work.columns:
        work["start_ms"] = (work["timestamp"].astype("int64") // 1_000_000).astype(np.int64)
...
def fetch_market_snapshot(symbol: str, base_url: str = "https://api.bybit.com") -> dict[str, Any]:
    ...
    if ticker_payload.get("retCode") != 0:
        raise RuntimeError(f"Bybit ticker error {symbol}: {ticker_payload}")
    ...
    orderbook_resp = requests.get(
        orderbook_endpoint,
        params={"category": "linear", "symbol": symbol, "limit": 1},
        timeout=30,
    )
    orderbook_resp.raise_for_status()
    orderbook_payload = orderbook_resp.json()
    if orderbook_payload.get("retCode") != 0:
        raise RuntimeError(f"Bybit orderbook error {symbol}: {orderbook_payload}")
...
def sync_market_data_to_sqlite(
    ...
    save_klines_to_sqlite(history_df, symbol=symbol, interval=sync_config.interval, db_path=db_path)

    snapshot = None
    if fetch_snapshot:
        snapshot = fetch_market_snapshot(symbol=symbol, base_url=sync_config.base_url)
        save_market_snapshot_to_sqlite(snapshot, db_path=db_path)
